...
if [[ -f "$GRADLEW" ]]; then
  chmod +x "$GRADLEW"
  # Try running gradle to trigger wrapper download (it auto-downloads if needed)
  GRADLE_VER=$(cd "$ANDROID_GEN" && ./gradlew --version --quiet 2>/dev/null | grep "Gradle " | awk '{print $2}' || echo "unknown")
  ok "Gradle  ${C}($GRADLE_VER)${N}"
else
...
npx @tauri-apps/cli android build "${TAURI_FLAGS[@]}" 2>&1 | tee "$BUILD_LOG_TAURI" | while IFS= read -r line; do
  ...
  if echo "$line" | grep -qE "(error|warning|Compiling|Finished|BUILD|Download|Progress|Gradle|Task|Configure|Transform|Link|Dex|Merge|Package|Sign|APK|AAB|\[=+)"; then
    # Gradle progress bar detection
    if echo "$line" | grep -qE '\[=+'; then
      pct=$(echo "$line" | grep -oE '[0-9]+%' | tr -d '%' | head -1)
      if [[ -n "$pct" ]]; then
        progress "Gradle build" "$pct" 100
        continue
      fi
    fi
    ...
    if echo "$line" | grep -qE "^Compiling "; then
      ...
      printf "\r  ${C}⟳${N}  Compiling %-40s" "$pkg"
      continue
    fi
    # Finished
    if echo "$line" | grep -qE "^Finished "; then
      echo ""
      ok "Rust compile finished"
      continue
    fi
    # Errors — show in red
    ...
    # General info
    if echo "$line" | grep -qE "(BUILD SUCCESSFUL|BUILD FAILED|Packaged|Signed|APK|AAB)"; then
      echo -e "  ${G}$line${N}"
      continue
    fi
    echo "    $line"
